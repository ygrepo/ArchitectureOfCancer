...
cancer_filenames <- getCancerFiles("data/")
...
for (study in studies) {
  ...
  # Print correlations for the current study
  cat(
    "Cancer:", study_results$cancer,
    "Correlation:", study_results$cor, "\n"
  )
}
...
expression_val <- expression("BRCA1: Correlations between " ~ beta ~
  " and FUSE Score by Cancer Type")
...
filename <- "20231015_BRCA1_BETA_FUSE_Correlations_Cancer.png"
file_path <- paste0(figure_path, filename)
# png(file_path, width = 800, height = 600, units = "px", pointsize = 12)
# print(pt)
# dev.off()
graph2png(x = pt, file = file_path, dpi = 600, aspectr = 2)
